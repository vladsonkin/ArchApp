...
import com.hamedsafari.remote.base.BaseTest
...
class UserServiceTest: BaseTest() {
    ...
    fun `search top users by name`() {
        ...
        runBlocking {
            ...
            assertEquals("hamedsilver", users.items.first().login)
    ...
    fun `fetch user's detail`() {
        ...
        runBlocking {
            val user = userService.fetchUserDetailsAsync("hamedsilver")
            assertEquals("6847959", user.id)
            assertEquals("hamedsilver", user.login)
            assertEquals("https://avatars0.githubusercontent.com/u/6847959?v=4", user.avatarUrl)
            assertEquals("hamed safari", user.name)
            assertEquals("https://medium.com/@hamedsafari", user.blog)
    ...
    fun `fetch user's detail and fail`() {
        ...
        runBlocking {
            userService.fetchUserDetailsAsync("hamedsilver")
        }
    }
}